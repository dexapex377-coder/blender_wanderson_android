                               ...
                               size_t *r_final_len)
{
  ...
  if (l_end > 0) {
    /* Not `.end() - str`: trim_right() returns a default-constructed StringRef when the
     * whole span is whitespace, and that one does not point into `str` at all, so the
     * subtraction yields the distance to an unrelated literal. Since the span starts at
     * `str` and only its right side is trimmed, the remaining length is the offset -- and
     * it is 0 when everything was trimmed, which is what this needs.
     *
     * Reached with a label like "    Brush" in a panel narrow enough that only its leading
     * spaces fit: an indented sub-panel label in portrait orientation. */
    l_end = int(StringRef(str, l_end).trim_right().size());
                          ...
                          const bool clip_right_if_tight)
{
  ...
  if ((okwidth > 0.0f) && (strwidth > okwidth)) {
        ...
        (l_end < 10 || min_ff(parts_strwidth, strwidth - okwidth) < minwidth))
    {
      ...
      /* See the note in text_clip_right_ex: an all-whitespace span trims to a StringRef
       * that does not point into `str`, so the length has to come from size(). */
      l_end = size_t(StringRef(str, l_end).trim_right().size());
