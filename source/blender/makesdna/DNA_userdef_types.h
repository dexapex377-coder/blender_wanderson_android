...
/* Interface sizing. A phone is held much closer than a monitor but has far less
 * room, so the factory interface is uncomfortable to read and its editor
 ...
 *
 * 0.5 was settled by drawing with it rather than derived, which is also why the
 * softness stays at Blender's own default: a gamma curve was tried and turned
 * out not to be needed once the threshold was right.
 *
 * This is an ordinary preference under Input > Tablet; only the starting point
 * differs. An existing userpref.blend keeps its own value, which is what the
 * versioning code in versioning_userdef.cc is for.
 *
...
struct UserDef {
  ...
  /** Viewport texture size limit, zero meaning unlimited. Defaults to a real size rather than
   * unlimited: on a mobile GPU a scene carrying a dozen 4K maps exhausts the shared graphics
   * budget before the first frame finishes. Only the engines that draw through a GPU texture are
   * affected, so EEVEE and Workbench; Cycles reads the image buffers directly. */
