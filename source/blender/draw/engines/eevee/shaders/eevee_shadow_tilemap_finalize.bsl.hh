                             ...
                             [[global_invocation_id]] const uint3 global_id)
{
  ...
  if (lod_valid_thread) {
    ...
    if (all(lessThan(relative_tile_co, viewport_size))) {
      /* DOWNSTREAM (Android): port upstream c394b602b67e. The rendermap is fully cleared to
       * invalid pages (0xFFFFFFFF) before this stage, so we only write valid pages here. */
